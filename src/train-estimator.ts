...
export class TrainTicketEstimator {
  ...
  private getTotalPriceForPassengers(passengers: Passenger[], basePrice: number, totalPrice: number, tripRequest: TripRequest) {
    for (let i = 0; i < passengers.length; i++) {
      ...
      const between1And4YearsOld = passenger.age > 0 && passenger.age < 4;
  ...
  private validTripRequestInput(tripRequest: TripRequest) {
    if (tripRequest.passengers.length === 0) return 0;
    else if (tripRequest.details.from.trim().length === 0) throw new InvalidTripInputException('Start city is invalid');
    else if (tripRequest.details.to.trim().length === 0) throw new InvalidTripInputException('Destination city is invalid');
    else if (tripRequest.details.when < new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDate(), 0, 0, 0)) throw new InvalidTripInputException('Date is invalid');

    tripRequest.passengers.forEach((passenger) => {
      if (passenger.age < 0) throw new InvalidTripInputException('Age is invalid');
    });
  }
  ...
  private getDiscountByDate(tripDate: Date) {
    ...
    if (tripDate >= thirtyDaysFromNow) return constant.TRIP_DATE_DISCOUNTS.MORE_THAN_30_DAYS;
    ...
    if (tripDate < fiveDaysAgo && tripDate <= sixHoursAgo) return constant.TRIP_DATE_DISCOUNTS.IN_LESS_THAN_6_HOURS;
    else return 1;
  }

  private getDiscountByAge(age: number) {
    if (age <= 17) return constant.AGE_DISCOUNTS.UNDER_17;
    else if (age >= 70) return constant.AGE_DISCOUNTS.OVER_70;
    else return constant.AGE_DISCOUNTS.OTHER;
  }

  private getDiscountByDiscountCard(passenger: Passenger, passengers: Passenger[]) {
    let totalDiscount = 0;
    if (passenger.lastName) {
      const familyDiscountCard = passengers.some((passenger) => passenger.discounts.includes(DiscountCard.Family) && passenger.lastName === passenger.lastName);

      if (familyDiscountCard) return constant.DISCOUNT_CARDS.FAMILY;
    }

    if (passenger.discounts.includes(DiscountCard.Senior) && passenger.age >= 70) totalDiscount += constant.DISCOUNT_CARDS.SENIOR;

    if (passengers.length == 2) {
      const coupleDiscountCard = passengers.some((p) => p.discounts.includes(DiscountCard.Couple));
      const areMajors = passengers.every((p) => p.age >= 18);

      if (coupleDiscountCard && areMajors) totalDiscount += constant.DISCOUNT_CARDS.COUPLE;
    }

    if (passengers.length == 1 && passenger.discounts.includes(DiscountCard.HalfCouple) && passenger.age > 18) totalDiscount += constant.DISCOUNT_CARDS.HALF_COUPLE;
